...
const {Home} = require('../models/schemas');
const {Favourite} = require('../models/schemas');
...
exports.getHomeDetails = (req, res, next) => {
    const Id = new mongoose.Types.ObjectId(req.params.homeID);
    console.log(`Requested home details are of : ${Id}`);    


    let promise = Home.findOne({_id:Id});
    promise.then((reqhome) => {
        res.render('home-details/home1', { homeID: Id, homeObj: reqhome });
    })
};
...
exports.addtofav = (req, res, next) => {
    ...
    // 1.) get home to be favouriteed from 'homes' collection
    const promise = Home.findOne({_id : Id});


    // 2.) create an instance of it as per 'favourite schema' and save it
    promise.then((reqHome)=>{
        const newFav = new Favourite({Id});
        newFav.save();
    })
...
exports.getFav = (req, res, next) => {
    const promise = Favourite.find();

    promise.then((allFav)=>{
        res.render(faV.ejs,{favHomes: allFav});
    })
}
